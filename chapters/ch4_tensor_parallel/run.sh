...
if [[ -z "${BASE_CLOCK}" ]]; then
    ...
    echo "  export BASE_CLOCK=<MHz>  # e.g. 1410 for PCIe H100" >&2
    exit 1
fi
...
# ── Generate agent_deep trace (idempotent; seed=42 produces a fixed trace) ─────
if [[ ! -f "${TRACE}" ]]; then
    echo "==> Generating agent_deep trace ..."
    agentperf-generate --preset agent_deep --output "${TRACE}"
fi

# ── Lock graphics clocks on both GPUs before the first server starts ────────────
...
# ── Helper: wait for the vLLM /health endpoint ─────────────────────────────────
wait_for_server() {
    local url="$1"
...
# ── Helper: 60-second warmup window ────────────────────────────────────────────
warmup_server() {
    local label="$1"; shift
